...
(function(FU) {
	...
	var EventPool = (function(){
		...
		var E = {
			 ...
			 */
			bind : function(fieldName) {
				$.each(_POOL, function(i, v) {
					var element;

					// 不重复绑定，且只对指定的字段绑定
					if (v.binded || (fieldName && v.field!=fieldName)) {
						return;
					}
...
// @todo 动态属性没有生效
	...
	FU.Construct.prototype.addGroup = function(groupName) {
		...
		$.each(this.config.items, function(i, fieldItem){
		...
		if (groupConfig) {
			...
			$(content).insertAfter($('.formui-group.'+ cloneConfig.name).last());
	...
	FU.Construct.prototype.rebuildField = function(fieldConfig) {
		...
		EventPool.bind(fieldConfig.name)
